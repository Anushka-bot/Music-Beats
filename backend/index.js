  ...
  spotifyApi2
    ...
    .then((data) => {
      ...
      setInterval(async () => {
        ...
        console.log("The access token has been refreshed!");
        console.log("access_token:", access_token);
        spotifyApi3.setAccessToken(access_token);
      }, (expires_in / 2) * 1000);
    })
    .catch((error) => {
      console.error("Error getting Tokens:", error);
...
///////////////////////////////////////////////////////////
var spotifyApi3 = new SpotifyWebApi({
  clientId: "",
  clientSecret: "",
  redirectUri: "http://localhost:5000/callback3",
});
app.get("/login3", (req, res) => {
  console.log(spotifyApi3.createAuthorizeURL(scopes));
  res.redirect(spotifyApi3.createAuthorizeURL(scopes));
});
app.get("/callback3", (req, res) => {
  const error = req.query.error;
  const code = req.query.code;
  const state = req.query.state;
  ...
  spotifyApi3
    ...
    .then((data) => {
      ...
      spotifyApi3.setAccessToken(access_token);
      spotifyApi3.setRefreshToken(refresh_token);
      newtoken = access_token; ////////
      console.log("access_token:", access_token);
      console.log("refresh_token:", refresh_token);
      ...
      res.redirect("http://localhost:3000/spotify_playlist");

      setInterval(async () => {
        const data = await spotifyApi3.refreshAccessToken();
        const access_token = data.body["access_token"];
        newtoken = access_token; /////////////////////////////////////////////////////////
        console.log("The access token has been refreshed!");
        console.log("access_token:", access_token);
...
app.post("/download3", async (req, res) => {
  ...
  console.log(token);
  ////////////////////////////////////////////////////////////////////
  spotifyApi3.setAccessToken(token);
  console.log(`Starting Download, This may take a while...`);
  let x = 0,
    y = 0;
  ...
  spotifyApi3
    ...
    .then(
      function (data) {
        console.log("Created playlist!");
        /////////////////////////////////
        spotifyApi3.addTracksToPlaylist(data.body.id, urls).then(
          function (data) {
            console.log("Added tracks to playlist!");
          },
          function (err) {
...
app.post("/party", (req, res) => {
  ...
  const num = `whatsapp:${number}`;
  giphy.search("HappyNewYear2023", function (err, res) {
    ...
    twilio.messages.create(
      {
        ...
        to: num,
        body: "hi",
        mediaUrl: gif.images.original_mp4.mp4,
      },
      function (err, data) {
        console.log(data);
        twilio.messages.create(
    ...
    console.log(gif.images.original_mp4.mp4);
  });
  res.json({ arr: "done" });
});
app.listen(5000, () => {
  console.log(
    "Server is running on http://localhost:5000/login && http://localhost:5000/login2&& http://localhost:5000/login3"
  );
});
